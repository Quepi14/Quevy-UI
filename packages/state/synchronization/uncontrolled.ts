 ...
 */
export function createUncontrolledState<T>(
    defaultValue: T,
): UncontrolledState<T> {
 ...
 */
export function createControllableValue<T>(
    defaultValue: T,
): ControllableValue<T> {
    const fallback = createUncontrolledState(defaultValue);

    function value(controlledValue: T | undefined): T {
        return isControlled(controlledValue) ? controlledValue : fallback.get();
    }

    function request(controlledValue: T | undefined, next: T): T {
